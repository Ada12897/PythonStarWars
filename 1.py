...
# ✅ ДОБАВИЛА ТОЛЬКО ЭТО: авто-масштаб под экран (вместо PLAYER_SCALE=0.75)
def _bbox_h(surf: pygame.Surface) -> int:
    r = surf.get_bounding_rect(min_alpha=1)
    return r.height if r.height > 0 else surf.get_height()

def compute_scale_for_target_height(frames, target_h_px: int) -> float:
    base_h = max(_bbox_h(fr) for fr in frames)
    base_h = max(1, base_h)
    return target_h_px / base_h

# --- PLAYER frames (ТВОИ ФАЙЛЫ И ТВОЯ ПОСЛЕДОВАТЕЛЬНОСТЬ НЕ ТРОГАЮ) ---
player_idle_raw = load_img("duku1.png")
player_walk_raw = [load_img("duku_2.png"), load_img("duku_3.png"), load_img("duku_4.png")]
player_attack_raw = [load_img("duku2.png"), load_img("duku3.png"), load_img("duku4.png"), load_img("duku5.png"), load_img("duku6.png")]

# ✅ ТОЛЬКО РАЗМЕР: подгоняем высоту персонажа под экран
PLAYER_TARGET_H = int(HEIGHT * 0.22)  # хочешь меньше/больше — меняем ТОЛЬКО это число (0.20..0.26)
PLAYER_SCALE = compute_scale_for_target_height([player_idle_raw] + player_walk_raw + player_attack_raw, PLAYER_TARGET_H)
