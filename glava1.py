...
# ==========================================================
#  SIZE TUNING — КАК ВО 2/3 ГЛАВЕ (✅ ОДИНАКОВО)
# ==========================================================
BOT_HEIGHT_MULT   = 1.35   # ✅ как во 3 главе
BASE_SCALE_PLAYER = 0.75   # ✅ как во 3 главе

# Босс — по той же логике что в 3 главе (target_h * 1.08)
BOSS_SCALE_MULT_FROM_TARGET_H = 1.08
...
# ==========================================================
INTRO_DIALOG = [
    ("Связь", "Набу под атакой. Дроиды прочёсывают дороги у города."),
    ("Джедай", "Держи линии. Переключай дорожки и не подпускай в упор."),
    ("ГГ", "Принял. Начинаю зачистку."),
    ("Связь", "A/D — движение, W/S — дорожка, ЛКМ — удар. SPACE — далее.")
]
WAVE2_DIALOG = [
    ("Связь", "Они усилили патруль. Вторая волна плотнее."),
    ("Джедай", "Не стой на месте. Руби тех, кто подошёл близко."),
    ("ГГ", "Понял.")
]
BOSS_DIALOG = [
    ("Связь", "Внимание… на поле выходит кто-то другой."),
    ("Дарт Мол", "Ты слишком долго мешаешь моим планам."),
...
# ==========================================================
class Player:
    def __init__(self, WIDTH, WORLD_WIDTH, lanes, PLAYER_Y_OFFSET,
...
class Bot:
    ...
    def update_ai(self, player: Player, bullets):
        ...
        # стрельба только если бот повернут к игроку
...
class BossMaul:
    def __init__(self, x, lane_index, WORLD_WIDTH, lanes, BOSS_Y_OFFSET,
                 boss_idle_right, boss_idle_left,
                 boss_walk_right, boss_walk_left,
                 boss_attack_right, boss_attack_left):
        self.x = float(x)
        self.lane_index = lane_index
        self.WORLD_WIDTH = WORLD_WIDTH
        self.lanes = lanes
        ...
        self.frame_walk = 0.0
        self.walk_speed = 0.14

        self.boss_idle_right = boss_idle_right
        self.boss_idle_left = boss_idle_left
        self.boss_walk_right = boss_walk_right
        self.boss_walk_left = boss_walk_left
        self.boss_attack_right = boss_attack_right
        self.boss_attack_left = boss_attack_left
    ...
    def _pick_mode(self, now, player):
        ...
        r = random.random()
        if dist > 360:
            self.mode = "dash" if r < 0.40 else "stalk"
            self.mode_until = now + random.randint(520, 820)
        elif dist > 200:
            self.mode = "feint" if r < 0.35 else "stalk"
            self.mode_until = now + random.randint(620, 980)
        else:
            self.mode = "stalk" if r < 0.80 else "dash"
    ...
    def update_ai(self, player: Player):
        ...
        if self.mode == "stalk":
            ...
            if dist > 180:
                direction = 1 if dx > 0 else -1
                self.x += direction * (self.base_speed * 1.02)
                self.facing_right = direction > 0
                self.moving = True
            else:
                direction = -1 if dx > 0 else 1
                self.x += direction * (self.base_speed * 0.92)
                self.facing_right = direction > 0
                self.moving = True
        ...
        if self.attacking:
            self.frame_hit += self.hit_speed
            hit_list = self.boss_attack_right if self.facing_right else self.boss_attack_left
            idx = int(self.frame_hit)
            if idx >= len(hit_list):
                self.frame_hit = 0.0
            ...
            if self.moving:
                self.frame_walk += self.walk_speed
                walk_list = self.boss_walk_right if self.facing_right else self.boss_walk_left
                idx = int(self.frame_walk) % len(walk_list)
                self.current_sprite = walk_list[idx]
            else:
                self.frame_walk = 0.0
                self.current_sprite = self.boss_idle_right if self.facing_right else self.boss_idle_left
...
def draw_boss_hp(screen, boss, FONT_SMALL, WIDTH, HEIGHT, name="ДАРТ МОЛ"):
    ...
    title = FONT_SMALL.render(name, True, (255, 120, 120))
    screen.blit(title, (x, y - 18))


# ==========================================================
#  DB BOTS PARSER (оставлено как было)
# ==========================================================
...
def run_chapter(character_id=None):
    ...
    pygame.display.set_caption("Star Wars — Chapter 1 (Naboo)")
    clock = pygame.time.Clock()

    bg = safe_load_image("background", fallback_size=(WIDTH, HEIGHT), log_tag="background.png")
    ...
    # ========= PLAYER LOAD (✅ теперь как во 2/3 главе)
    ...
    # высота-эталон берется с Энакина * BASE_SCALE_PLAYER (как в 3 главе)
    ...
    # сам персонаж: BASE_SCALE_PLAYER, бултар подгоняем по bbox как раньше
    if char_key == "bultar":
    ...
    def build_bot_set(prefix: str, player_bbox_h_px: float, height_mult: float):
        bot_raw = [safe_load_image(f"{prefix}{i}.png", log_tag=f"{prefix}{i}.png") for i in range(0, 8)]
    ...
    bot_set_yellow = build_bot_set("yellow", player_bbox_px, BOT_HEIGHT_MULT)
    bot_set_red    = build_bot_set("red",    player_bbox_px, BOT_HEIGHT_MULT)

    # ========= BOSS MAUL (✅ теперь по логике как в 3 главе)
    boss_idle_raw = safe_load_image("mal1", log_tag="mal1")
    boss_walk_raw = [safe_load_image(n, log_tag=n) for n in ["mal2", "mal3", "mal4"]]
    boss_attack_raw = [safe_load_image(n, log_tag=n) for n in ["mal_lick", "mal_lick2"]]

    boss_bbox_h0 = max(1, sprite_bbox_h(boss_idle_raw))
    BOSS_SCALE = clamp((target_h / boss_bbox_h0) * float(BOSS_SCALE_MULT_FROM_TARGET_H), 0.35, 1.60)

    BOSS_CANVAS = bbox_size_after_scale([boss_idle_raw] + boss_walk_raw + boss_attack_raw, BOSS_SCALE)
    ...
    player = Player(WIDTH, WORLD_WIDTH, lanes, PLAYER_Y_OFFSET,
    ...
    # ========= волны (2 волны + босс)
    waves = [
        {"kills_required": 10, "spawn_min": 2, "spawn_max": 3},  # волна 1: yellow
        {"kills_required": 14, "spawn_min": 2, "spawn_max": 3},  # волна 2: red
    ]
    wave_idx = 0
    killed_in_wave = 0
    ...
    def spawn_pack():
        ...
        # строго по волнам:
        # wave 0 -> yellow, wave 1 -> red
        if wave_idx == 0:
            bot_walk_right_imgs, bot_walk_left_imgs, bot_idle_left_img, BOT_Y_OFFSET = bot_set_yellow
            bot_hp, bot_dmg, bot_spd = 55, 6, 2.10
        else:
            bot_walk_right_imgs, bot_walk_left_imgs, bot_idle_left_img, BOT_Y_OFFSET = bot_set_red
            bot_hp, bot_dmg, bot_spd = 65, 7, 2.20
    ...
    def draw_world_only():
        ...
        if boss_spawned and boss is not None and boss.alive and boss_visible:
            draw_boss_hp(screen, boss, FONT_SMALL, WIDTH, HEIGHT, name="ДАРТ МОЛ")
    ...
    # последний кадр (final.py сам покажет попапы)
    draw_world_only()
    pygame.display.flip()
...
if __name__ == "__main__":
    w, frame = run_chapter(character_id=1)
    print("WON:", w)
